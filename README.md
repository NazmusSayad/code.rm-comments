...
This is a fork of [rioj7/remove-comments](https://github.com/rioj7/remove-comments), which is a rewrite of the extension [Remove Comments](https://marketplace.visualstudio.com/items?itemName=plibither8.remove-comments) by [plibither8](https://github.com/plibither8).

This version has been completely rewritten with:

- **TypeScript** - Full type safety with strict mode enabled
- **Modern tooling** - pnpm + tsdown bundling
- **Clean architecture** - Class-based language definitions organized by category
...
If you have defined the setting [`rm-comments.keep`](#rm-commentskeep) you can ignore this setting with the command  
**Comments: Ignore any "rm-comments.keep" setting. For next removal only.**  
In **only the next call** of the Remove commands the `rm-comments.keep` setting is not used.
...
The command `rm-comments.removeAllCommentsWithPrefix` can be used in a keybinding. You can pass the prefix in the argument:
...
If you want to extract the strings for a large collection of files you can use the extension: [Command on All Files](https://marketplace.visualstudio.com/items?itemName=NazmusSayad.commandOnAllFiles)

# Settings

## `rm-comments.keep`

With the setting `rm-comments.keep` you are able to define which comments to keep by testing the comment text with a Regular Expression. The comment text is all the text of the comment without the comment delimiters. For a block comment (like `/* ... */`) the lines are separated with a Tab character. The _Indent Comment Continuation_ lines are not part of the comment text, but have the same action (keep/remove) as the first line.

The setting `rm-comments.keep` is an object with the keys a [languageId](https://code.visualstudio.com/docs/languages/overview#_language-id). You can use any known VSC languageId and the special languageId `all`. The `all` key is used for any file.
